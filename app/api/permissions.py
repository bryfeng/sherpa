...
class PermissionRequestModel(BaseModel):
    ...
    session_key_address: Optional[str] = Field(
        default=None,
        description="Session key address (generated by frontend using Rhinestone SDK). If not provided, backend returns config for frontend to generate one.",
    )
...
async def request_permission_grant(
    ...
    This endpoint:
    1. Takes session_key_address from frontend (generated via Rhinestone SDK)
    2. Builds the Smart Session configuration with spending limits and constraints
    ...
    No backend signing is needed - Smart Session grants authorize future intents on-chain.
    ...
    # Session key address is provided by frontend (generated via Rhinestone SDK)
    # If not provided, return a placeholder that frontend must fill in
    session_key_address = request.session_key_address or "FRONTEND_MUST_GENERATE"
    ...
    # Validate the configuration (skip if placeholder address)
    if session_key_address != "FRONTEND_MUST_GENERATE":
